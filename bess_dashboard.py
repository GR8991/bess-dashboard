...
Three data source modes:
  1. Demo mode     — built-in synthetic data, works instantly with no files
  2. URL mode      — paste the NLR dataset zip URL, downloads automatically
  3. Local mode    — paste local folder path (for local use only)
...
NLR_ZIP_URL = "https://data.nlr.gov/system/files/312/1774982010-dataset.zip"

# ─────────────────────────────────────────────────────────────────────────────
# DEMO DATA GENERATOR
# Synthetic data that mirrors real NLR power profiles
# Used when no dataset is available (Streamlit Cloud demo mode)
# ─────────────────────────────────────────────────────────────────────────────
def generate_demo_data():
    """
    Generate synthetic power profiles that mirror real NLR data structure.
    Returns (meta_df, data_df) ready for analysis.
    """
    np.random.seed(42)
    dt      = 0.2
    configs = [
        # (nodes, model, repeat, pattern)
        (2,  "llama2_70b_lora",   0, "train"),
        (2,  "llama2_70b_lora",   1, "train"),
        (4,  "llama2_70b_lora",   0, "train"),
        (4,  "llama2_70b_lora",   1, "train"),
        (8,  "llama2_70b_lora",   0, "train"),
        (8,  "stable_diffusion",  0, "train"),
        (16, "llama2_70b_lora",   0, "train"),
        (16, "stable_diffusion",  0, "train"),
    ]

    meta_rows = []
    all_runs  = []

    for idx, (nodes, model, repeat, pattern) in enumerate(configs):
        # generate realistic power profile
        duration_s = np.random.uniform(4000, 6000)
        n_steps    = int(duration_s / dt)
        time_s     = np.arange(n_steps) * dt

        # per-node peak power based on real H100 measurements
        # 4 GPUs × ~700W + 2 CPUs × ~360W ≈ 3520W per node
        ...
        for i in range(n_steps):
            if i < ramp_steps:
                # steep ramp up — characteristic of synchronised GPU start
                frac      = i / ramp_steps
                power[i]  = idle_total + (peak_total - idle_total) * frac
                # add ramp noise
                power[i] += np.random.normal(0, peak_total * 0.01)
            elif i < plateau_end:
                # sustained plateau with small fluctuations
                power[i] = peak_total * np.random.uniform(0.94, 0.99)
            else:
                # abrupt ramp down
                frac      = (i - plateau_end) / (n_steps - plateau_end)
...
# URL / ZIP LOADER
# ─────────────────────────────────────────────────────────────────────────────
def resolve_url_source(url: str) -> pathlib.Path:
    """Download zip from URL, extract to temp folder, return dataset root."""
    import requests
    tmp = pathlib.Path(tempfile.mkdtemp()) / "nlr_dataset"
    tmp.mkdir(parents=True, exist_ok=True)

    prog = st.progress(0, text="⬇️  Downloading dataset …")
    ...
    for chunk in resp.iter_content(chunk_size=2 * 1024 * 1024):
        ...
        if total:
            ...
            mb  = done // (1024 * 1024)
            prog.progress(pct, text=f"⬇️  Downloading … {mb} MB / {total//1024//1024} MB")
    prog.empty()
...
def show_single_batch(run_df, file_num, grp_val, grp_lbl, dt, meta_row=None):
    ...
    with p3b:
        ...
        fig7.add_trace(go.Histogram(
            x=ramp_up, nbinsx=40,
            marker_color="#E24B4A", opacity=0.75, name="Up"),
            row=1, col=1)
        fig7.add_trace(go.Histogram(
            x=ramp_down, nbinsx=40,
            marker_color="#378ADD", opacity=0.75, name="Down"),
            row=1, col=2)
        fig7.update_layout(
...
with st.sidebar:
    ...
    source_mode = st.radio(
        "Choose data source",
        options=["demo", "url", "local"],
        format_func=lambda x: {
            "demo" : "🎯 Demo mode (synthetic data — no files needed)",
            "url"  : "🌐 NLR dataset URL (downloads zip)",
            "local": "💾 Local folder path (your PC only)",
        }[x],
        index=["demo","url","local"].index(
    ...
    if source_mode == "demo":
        st.info(
            "Demo mode uses synthetic power profiles that mirror "
            "real NLR H100 measurements. No files needed — works "
            "instantly on Streamlit Cloud."
        )
        ...
        st.markdown("**Dataset URL**")
        url_input = st.text_input(
            "Paste zip URL",
            value=NLR_ZIP_URL,
            help="Default is the official NLR dataset URL",
        )
        st.caption(
            "⚠️  The full NLR dataset is ~1 GB. "
            "Download may take 3–10 minutes depending on connection speed."
        )
        if st.button("⬇️  Download & load", type="primary",
                     use_container_width=True):
            try:
                base = resolve_url_source(url_input.strip())
                ...
                if found:
                    ...
                    st.error("❌  No workloads found in downloaded zip")
            except Exception as e:
    ...
    # ── Step 2: workload selector (only for real data) ────────────────────────
    ...
    if not is_demo and wl_found:
        ...
        if source_mode != "demo":
            st.info("Set data source above and load data first.")

    # ── Step 3: BESS parameters ───────────────────────────────────────────────
    st.divider()
...
# find the most recently loaded dataset
active_key = None
if source_mode == "demo" and "demo" in wl_cache:
    active_key = "demo"
elif wl_found:
    # use last loaded workload
...
if not active_key:
    ...
    st.markdown(
        ...
        This dashboard analyses AI workload power profiles from the
        **NLR Kestrel HPC** dataset (DOI: 10.7799/3025227) and computes
        BESS engineering parameters for grid integration with renewables.

        **Get started — choose a data source in the sidebar:**

        | Mode | Description | Best for |
        |---|---|---|
        | 🎯 Demo | Synthetic H100 profiles | Quick explore, Streamlit Cloud |
        | 🌐 URL | Download real NLR dataset | Full analysis, ~1 GB download |
        | 💾 Local | Your extracted zip folder | Local PC use |

        Then click **Load** and the dashboard will populate with all charts.
        """
    )
    st.stop()
...
# PAGE HEADER
# ─────────────────────────────────────────────────────────────────────────────
mode_badge = "🎯 Demo mode" if is_demo else f"📊 {wl_found.get(folder, folder)}"
st.title("⚡ BESS Engineering Dashboard")
st.caption(
    f"{mode_badge}  |  "
    f"{data['file_num'].nunique()} runs  |  "
    f"dt={dt:.2f} s  |  "
    f"{facility_nodes} nodes | PUE {pue} | {solar_frac}% solar"
)

if is_demo:
    st.info(
        "🎯 **Demo mode** — synthetic data generated to match real "
        "NLR H100 power profiles. Switch to URL or local mode in the "
        "sidebar to load real measurement data."
    )
    ...
    with r3b:
        ...
        for m in ["Peak (kW)","Mean (kW)","Idle (kW)"]:
            fig_bar.add_trace(go.Bar(
                x=agg["Group"].astype(str), y=agg[m].round(2), name=m))
...
with tab2:
    ...
    st.caption(
        "Select any batch below to see a full detailed analysis "
        "of that individual run — time-series, ramp, histogram, "
        "cumulative energy, power phases, and load duration curve."
    )
...
st.caption(
    ...
    "BESS sizing is indicative — requires full power system study for final design"
)
