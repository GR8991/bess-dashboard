...
Three data source modes:
  ...
  2. Google Drive  — downloads real NLR dataset zip automatically
  3. Local mode    — paste local folder path (for local PC use only)
...
def generate_demo_data():
    ...
    for idx, (nodes, model, repeat, _) in enumerate(configs):
...
# ─────────────────────────────────────────────────────────────────────────────
# GOOGLE DRIVE DOWNLOADER  — handles large file confirmation automatically
# ─────────────────────────────────────────────────────────────────────────────
...
with st.sidebar:
    ...
    if source_mode == "demo":
        st.info(
            "Synthetic H100 power profiles — mirrors real NLR measurements. "
            "Works instantly on Streamlit Cloud with no downloads."
        )
    ...
    # ── Google Drive ──────────────────────────────────────────────────────────
        ...
        st.info(
            f"Your dataset is pre-configured from Google Drive.\n\n"
            f"File ID: `{GDRIVE_FILE_ID[:20]}…`\n\n"
            f"Click below to download and extract (~1 GB). "
            f"This may take 5–15 minutes depending on connection speed."
        )
    ...
    # ── Workload selector (real data only) ────────────────────────────────────
...
if not active_key:
    ...
    **Get started — choose a data source in the sidebar ←**
    ...
    | ☁️ Google Drive | Real NLR dataset (1 GB) | 5–15 min download |
    | 💾 Local | Your extracted folder | Instant (local PC only) |
    """)
    st.stop()
...
# HEADER + KPIs
# ─────────────────────────────────────────────────────────────────────────────
mode_badge = "🎯 Demo" if is_demo else f"☁️ {wl_found.get(folder, folder)}"
st.title("⚡ BESS Engineering Dashboard")
st.caption(
...
if is_demo:
    st.info(
        ...
        "Switch to **Google Drive** in the sidebar to load real NLR data."
    )
...
st.caption(
    ...
    "DOI 10.7799/3025227  |  "
    "Solar: synthetic (sinusoidal) — replace with NSRDB data for Golden, CO  |  "
