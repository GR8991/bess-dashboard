...
#=============================================================================
YOU NED TO CONSIDDER FROM HERER FOR CODE VERSION 2
...
def _stats(data, dt):
    ...
    for (g, fid), chunk in data.groupby(["group","file_num"]):
        ...
        ramp  = pw.diff().abs() / max(dt, 1e-9)
        rows.append({
            "Group"   : g,   "File #": int(fid),
            "Peak kW" : round(pw.max() /1000,2),
            "Mean kW" : round(pw.mean()/1000,2),
            "Idle kW" : round(pw.quantile(.05)/1000,2),
            "Std kW"  : round(pw.std() /1000,3),
            "LF %"    : round(pw.mean()/pw.max()*100,1),
            "Dur s"   : round(chunk["time_s"].max(),1),
            "kWh"     : round(pw.sum()*dt/3600/1000,3),
            "R95 kW/s": round(ramp.quantile(.95)/1000,4),
            "Rmax kW/s":round(ramp.max()/1000,4),
        })
    return pd.DataFrame(rows)
...
def _batch_panel(run, fnum, gval, glbl, dt, mrow=None):
    ...
    with c4:
        ...
        fig4.update_layout(title="Cumulative energy",xaxis_title="min",
            yaxis_title="kWh",height=260,hovermode="x unified",
...
with st.sidebar:
    ...
    if src=="demo":
        ...
        _lp=st.text_input("Folder path",value=r"C:\Users\gangaraju.pilly\Downloads\dataset")
...
if _ok:
    ...
    top   = int(sdf.sort_values("Peak kW",ascending=False).iloc[0]["File #"])
    ...
    t1,t2,t3,t4,t5=st.tabs([
        "📊 All batches","🔬 Single batch","🔍 Node deep-dive",
    ...
    with t1:
        ...
        with b2:
            agg=sdf.groupby("Group")[["Peak kW","Mean kW","Idle kW"]].mean().reset_index()
            figBr=go.Figure()
            for m in ["Peak kW","Mean kW","Idle kW"]:
                figBr.add_trace(go.Bar(x=agg["Group"].astype(str),y=agg[m].round(2),name=m))
            figBr.update_layout(title="Peak/mean/idle",barmode="group",
                yaxis_title="kW",height=280,margin=dict(l=55,r=15,t=50,b=45))
            st.plotly_chart(figBr,use_container_width=True)
        with b3:
            lfa=sdf.groupby("Group")["LF %"].agg(["mean","std"]).reset_index()
            figLF=go.Figure()
            figLF.add_trace(go.Bar(x=lfa["Group"].astype(str),y=lfa["mean"].round(1),
                error_y=dict(type="data",array=lfa["std"].round(1)),
                marker_color=[gcols[g] for g in lfa["Group"]],
        ...
        st.dataframe(sdf.sort_values(["Group","File #"]),
    ...
    with t5:
        ...
        if len(sel)<2:
            ...
            with e2:
                ...
                mets=["Peak kW","Mean kW","LF %","R95 kW/s"]
                figM=go.Figure()
